    ...
    return (
        <DashboardLayout>
            <div className="mx-auto max-w-2xl space-y-6">
                <div>
                    <h2 className="text-xl font-bold text-[#F0EBE1]">Create Donation Listing</h2>
                ...
                <section className="rounded-xl border border-white/10 bg-white/5 p-5">
                    <form className="grid grid-cols-1 gap-4 md:grid-cols-2" onSubmit={handleCreateDonation} noValidate>
                        <div>
                            <label htmlFor="foodType" className={LABEL_CLASS}>Food Type</label>
                            <input id="foodType" type="text" value={form.foodType} onChange={(e) => handleFieldChange("foodType", e.target.value)} className={INPUT_CLASS} placeholder="Fresh bread" />
                            {errors.foodType ? <p className="mt-1 text-xs font-semibold text-rose-300">{errors.foodType}</p> : null}
                        </div>

                        <div>
                            <label htmlFor="quantity" className={LABEL_CLASS}>Quantity</label>
                            <input id="quantity" type="number" min="0.01" step="0.01" value={form.quantity} onChange={(e) => handleFieldChange("quantity", e.target.value)} className={INPUT_CLASS} placeholder="20" />
                        ...
                        <div>
                            ...
                            <input id="unit" type="text" value={form.unit} onChange={(e) => handleFieldChange("unit", e.target.value)} className={INPUT_CLASS} placeholder="kg, servings, boxes" />
                            {errors.unit ? <p className="mt-1 text-xs font-semibold text-rose-300">{errors.unit}</p> : null}
                        </div>

                        <div>
                            <label htmlFor="expirationDate" className={LABEL_CLASS}>Expiration Date</label>
                            <input id="expirationDate" type="date" value={form.expirationDate} onChange={(e) => handleFieldChange("expirationDate", e.target.value)} className={INPUT_CLASS} />
                            {errors.expirationDate ? <p className="mt-1 text-xs font-semibold text-rose-300">{errors.expirationDate}</p> : null}
                        </div>

                        <div className="md:col-span-2">
                            <label htmlFor="pickupAddress" className={LABEL_CLASS}>Pickup Address</label>
                            <input id="pickupAddress" type="text" value={form.pickupAddress} onChange={(e) => handleFieldChange("pickupAddress", e.target.value)} className={INPUT_CLASS} placeholder="No. 12, Main Street, Colombo" />
                            {errors.pickupAddress ? <p className="mt-1 text-xs font-semibold text-rose-300">{errors.pickupAddress}</p> : null}
                        </div>

                        <div>
                            <label htmlFor="availabilityTime" className={LABEL_CLASS}>Availability Time</label>
                            <input id="availabilityTime" type="time" value={form.availabilityTime} onChange={(e) => handleFieldChange("availabilityTime", e.target.value)} className={INPUT_CLASS} />
