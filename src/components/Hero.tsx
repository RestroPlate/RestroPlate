...
export function Hero() {
	...
	useEffect((): (() => void) => {
		const interval = setInterval((): void => {
			...
			setTimeout((): void => {
				setCurrentIndex(
					(prev: number): number => (prev + 1) % HERO_GALLERY.length,
				);
	...
	return (
		...
		>
			...
			<div
				...
				style={{
					background:
						"linear-gradient(135deg, rgba(11,26,8,0.88), rgba(5,15,3,0.55))",
				}}
			/>
			...
			<div
				...
				style={{
					background:
						"linear-gradient(to bottom, transparent, #7DC542, transparent)",
				}}
			/>

			{/* Content wrapper */}
			<div className="relative z-[3] flex-1 flex flex-row items-center justify-between w-full max-w-[1200px] mx-auto px-[5vw] pt-[100px] pb-[60px] gap-[4vw] box-border">
				{/* LEFT column */}
				<div
				...
				>
					...
					<p className="text-[1.05rem] leading-[1.72] tracking-[0.01em] text-[rgba(240,235,225,0.55)] max-w-[520px] mb-10 mt-5">
						We bridge the gap between restaurants and homes with surplus food
						and community members facing food insecurity — one meal, one
				...
				>
					...
					<div
						...
						style={{
							background:
								"linear-gradient(to top, rgba(11,26,8,0.45) 0%, transparent 50%)",
						}}
					/>
						...
						{HERO_GALLERY.map((_: string, i: number) => (
							<div
								...
								style={{
									...
									background:
										i === currentIndex ? "#7DC542" : "rgba(255,255,255,0.35)",
									transform:
										hoveredDot === i && i !== currentIndex
											? "scale(1.2)"
											: "scale(1)",
								}}
							/>
