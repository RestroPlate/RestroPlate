...
export async function createClaim(donationId: number): Promise<DonationClaim> {
	try {
		...
		throw new Error(
			extractErrorMessage(err, "Failed to create claim. Please try again."),
		);
	}
}
...
export async function getMyClaims(): Promise<DonationClaim[]> {
	try {
		const { data } = await apiClient.get<
			ClaimApiResponse[] | Record<string, unknown>
		>("/api/donation-claims/my");
		return extractClaimsList(data).map(mapClaimResponse);
	} catch (err) {
		throw new Error(
			extractErrorMessage(err, "Failed to load claims. Please try again."),
		);
	}
}
...
export async function updateClaimStatus(
	...
	try {
		const { data } = await apiClient.patch<ClaimApiResponse>(
			`/api/donation-claims/${claimId}/status`,
			{
				status,
			},
		);
		...
		throw new Error(
			extractErrorMessage(err, `Failed to ${status.toLowerCase()} claim.`),
		);
	}
}
